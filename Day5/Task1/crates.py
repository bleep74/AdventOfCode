...
for line in Lines:
	...
	while count < int(motions[1]):
		letter = crates[int(motions[3])-1][len(crates[int(motions[3])-1]) - 1]
		crates[int(motions[3])-1].pop(len(crates[int(motions[3])-1]) - 1)
		crates[int(motions[5])-1].append(letter)
		count +=1
